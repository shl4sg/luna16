...
import remodel.senet

def se_dense_block(x, blocks, name):
    """A se-dense block.
    密集的模块
    # Arguments
    参数
    ...
    for i in range(blocks):
        x = se_conv_block(x, 32, name=name + '_block' + str(i + 1))
    return x




def se_conv_block(x, growth_rate, name,):
    """A building block for a se-dense block.
    密集块正在建立的块
    # Arguments
    参数
        x: input tensor.
    ...
    x1 = remodel.senet.squeeze_excitation_layer(x1,growth_rate,16)
...
def densenet(growth_rate=32,classes=2):
    ...
    x = se_dense_block(x, blocks[0], name='conv2')
